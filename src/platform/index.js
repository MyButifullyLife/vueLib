...
var functions = {
		...
		return new Promise(function (resolve, reject) {
			if (navigator.camera) {
				try {
					...
					// cameraOptions.sourceType =  navigator.camera.PictureSourceType.CAMERA; // 设置打开相机0  还是相册 1
					...
					cameraOptions.quality = cameraOptions.quality || 50; // 照片压缩比例 [1, 100]
					cameraOptions.targetWidth = cameraOptions.targetWidth || 640;
					cameraOptions.targetHeight = cameraOptions.targetHeight || 960;
    ...
    /**
     * @description 禁止ios滑动整个页面
     * @returns {Promise}
     * * 0 表示禁止滑动，1 表示允许滑动
     */
	setBounces: function(flag) {
		return functions.callApi('MideaCommon', 'setBounces', [flag]);
	},
    /**
     * 获取位置信息
     * @return {*|promise}
    ...
    getLocation: function() {
        return functions.callApi(_MIDEA_MAP, 'location', ['1', 'ThreeKilometers']);
    }
};
